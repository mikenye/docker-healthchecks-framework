...
function check_tcp4_connection_established_for_pid() {
    # $1 = local IP
    # $2 = local port
    # $3 = remote IP
    # $4 = remote port
    ...
    regex="^tcp\s+\d+\s+\d+\s+${regex_local_ip}:${regex_local_port}\s+${regex_remote_ip}:${regex_remote_port}\s+ESTABLISHED\s+${regex_pid_process_name}\s*$"
    
    # Check to see if the connection is established
    if netstat -anp | grep -P "$regex" > /dev/null 2>&1; then
      >&2 echo "TCP4 connection between $1:$2 and $3:$4 for PID $5 established: PASS"
      true
    else
      >&2 echo "TCP4 connection between $1:$2 and $3:$4 not PID $5 established: FAIL"
      false
    fi
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  check_tcp4_connection_established_for_pid "$1" "$2" "$3" "$4" "$5"
fi
