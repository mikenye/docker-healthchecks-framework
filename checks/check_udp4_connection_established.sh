...
function check_udp4_connection_established() {
    # $1 = local IP
    # $2 = local port
    # $3 = remote IP
    # $4 = remote port
    ...
    regex="^udp\s+\d+\s+\d+\s+${regex_local_ip}:${regex_local_port}\s+${regex_remote_ip}:${regex_remote_port}\s+ESTABLISHED\s*$"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  check_udp4_connection_established "$1" "$2" "$3" "$4"
fi
